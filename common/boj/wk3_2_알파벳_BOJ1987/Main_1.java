...
public class Main_1 {
	...
	static class State{ // BFS에서 현재 탐색중인 상태를 저장하기 위한 클래스
		int r,c; // 좌표
		Set<Character> way = new HashSet<>(); // 현재 위치 포함 지금까지 밟은 Character 저장
		State(int r,int c,Set<Character> way){
			this.r=r;
			this.c=c;
			this.way=way;
		}
	...
	public static void main(String[] args) throws IOException {
		...
		startSet.add(map[1][1]);
		State start = new State(1,1,startSet);// 좌측 상단 1,1 에서 시작
		...
		while(!q.isEmpty()) {
			...
			for (int r = 0; r < q.size(); r++) {// 현재 들어가 있는 큐는 같은 거리에 있음			
				...
				for (int i = 0; i < 4; i++) {// 현재 밟은 자리로부터 4방 탐색
					...
					if(nChar != NULL && !current.way.contains(nChar)) {//어차피 입력은 전부 대문자라서 NULL 검사 할 필요 없음, 현재까지 밟은 거랑 같은지 검사
						//처음 밟는 대문자일 때
						Set<Character> nWay = new HashSet<>(current.way);//현재까지 밟은 기록 복사(참조 X)
						nWay.add(nChar);//처음 밟는 문자 추가				
